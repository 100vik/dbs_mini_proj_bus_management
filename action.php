...
///////////////////////////////////////////////admin_check_booking_ with js main1_ajax////////////////////////////////////////////////
                                                  if (isset($_POST['bkd'])) {
                                                    $bid=$_POST['bid'];
                                                    $sql2="SELECT * FROM bus_details WHERE bus_id='$bid'";
                                                            $run2=mysqli_query($con,$sql2);
...
<?php
if (isset($_POST['calculate_revenue'])) {
    $bus_id = $_POST['bus_id'];
    $admin_name = $_POST['admin_name'];
    // Connect to database and execute CalculateTotalRevenue procedure
    $con = mysqli_connect('localhost', 'root', '', 'transport');
    if (!$con) {
    ...
    $sql = "CALL CalculateTotalRevenue($bus_id, @p_total_revenue)";
    if (mysqli_query($con, $sql)) {
        $sql_result = mysqli_query($con, "SELECT @p_total_revenue AS total_revenue");
        $row = mysqli_fetch_assoc($sql_result);
        $total_revenue = $row['total_revenue'];
        echo "<h2>Total Revenue for Bus ID $bus_id: $total_revenue</h2>";
    } else {
...
// Define the bus_id and journey_date
$bus_id = 123; // Replace 123 with the actual bus ID
$journey_date = '2024-04-18'; // Replace '2024-04-18' with the actual journey date

// Call the stored procedure using mysqli_query
$sql = "CALL GetBusTravelDetails($bus_id, '$journey_date')";
...
if ($result) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        echo "Passenger Name: " . $row['Passenger_Name'] . "<br>";
        echo "Seat No: " . $row['Seat_No'] . "<br>";
        echo "Number of Seat: " . $row['Number_of_Seat'] . "<br>";
        echo "Fare: " . $row['Fare'] . "<br>";
        echo "Status: " . $row['Status'] . "<br>";
        echo "Journey Date: " . $row['Journey_Date'] . "<br><br>";
    }
} else {
